...
**Side margins are a floor, not an addition.** `ScriptMargins.inset(safeArea:margin:rail:)` is the one place this is decided: the reader's margin applies where it exceeds the safe-area inset, and the inset wins otherwise, because content cannot be drawn under a cutout however narrow the margins are set. Adding the two is the bug this replaced — it cost the script ~73pt a side in landscape. The rail is added on top of both, since it is occupied space rather than a margin.
