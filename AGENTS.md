...
**The pursuit ceiling is measured in lines per second, not words per minute.** `ScrollPursuit.speed` takes the smaller of a term proportional to the gap and a ceiling of `lineHeight * pursuitMaxLinesPerSecond`. The ceiling's job is to stop the script teleporting, and that is a *perceptual* limit — how much text may sweep past before the reader loses their place — so it is expressed in the unit the eye tracks in. It used to be `pointsPerWord * effectiveWPM/60 * maxCatchUp`, and both inputs were unstable: `effectiveWPM` is a whole-take average (`wordsRead / elapsed`, floored at 60) whose `wordsRead` is the *cursor*, so it collapses during any pause taken without hitting Pause and while ad-libbing; and `pointsPerWord` divided a lookahead window's height by a word count, so it was really counting line breaks — quantised, and swinging fourfold as the cursor crossed them. Instrumented on a 16e across one take, the speed limit ranged from **1.2 to 4.7 lines a second** with nothing about the reading changed, and catch-up got slower the longer a take ran. That was the "laggy, smoother in 1.3" report. `lineHeight` now comes from the active word's measured frame plus `PrompterView.scriptLineSpacing` — **the same constant `FlowLayout` draws with, deliberately shared, because if the two drift the ceiling is quietly wrong and nothing on screen says so.** The estimates and the wpm readout still use `effectiveWPM`; that is what it is for.

`pursuitMaxLinesPerSecond = 4` is a starting point from the middle of the observed range, not a defended value — it was binding on 20-84% of frames on the run that motivated the change, and a ceiling active most of the time sets the speed rather than guarding it. `ScrollPursuitTests` pins the crossover (`response * linesPerSecond`, about three quarters of a line) and asserts that recovery takes the same time in *rows* at any type size, so re-tuning shows up there.

**Recognition falls back down a ladder, not off a cliff.** `SpeechTracker.Stage` tries on-device with contextual strings, then on-device without them, then server-based, advancing only when a rung produces *no result at all*. It used to be one `preferOnDevice` flag that latched false after a single failure and stayed there for the session. That was wrong on real hardware: a 16e with Siri and Dictation both enabled — and with on-device dictation demonstrably working in Notes under Airplane Mode — failed the first request with `kLSRErrorDomain 201` ("Siri and Dictation are disabled", which is generic and covers several causes) and then spent every take on server-based recognition. That is 300-800ms against 30-50ms, invisible in the UI, and it reads to the user as scrolling that lags behind their voice. `contextualStrings` is the second rung because it is the request's only unusual ingredient. **`supportsOnDeviceRecognition` returning `true` does not mean an on-device request will succeed** — never treat it as a precondition that settles anything.

**One version number, read by both bundles.** `MARKETING_VERSION` is set once in `project.yml`'s project-wide `settings.base`, and both `Cue/Info.plist` and `CueShare/Info.plist` carry `$(MARKETING_VERSION)` rather than a literal. App Store Connect rejects an upload whose extension and app disagree — error 90473 — and 1.5.2 was rejected exactly that way after only the app's number was bumped, because `CueShare/Info.plist` is a static checked-in file that `project.yml`'s `info.properties` does not touch. The same class of bug already forced `ci_pre_xcodebuild.sh` to stamp `CFBundleVersion` into *both* bundles. **When bumping a version, change `MARKETING_VERSION` and nothing else — then check the built product, not the source:** a plist substitution that fails ships the literal `$(MARKETING_VERSION)` as the version string, which reads fine in the repo and is nonsense on the device.

**The App Store's marketing and support URLs point at `docs/`, served by GitHub Pages.** They used to point at the repository root, which handed anyone who had just paid for the app a source tree. `docs/index.html` is the marketing page and `docs/support/index.html` the support page, sharing `docs/style.css` — the app's own palette (`#07090C` ground, `#FFB83B` the only accent) and the same condensed uppercase chrome type, so the site and the product read as one thing. Screenshots in `docs/img/` are downscaled copies of `Marketing/AppStoreScreenshots/` (`sips -Z 900`), because the App Store originals are 1320x2868 and far too heavy for a web page. **Keeping the pages in the repo is deliberate: an App Store URL cannot change without a new version submission, but the page behind it can be fixed with a commit.** The support page's FAQ carries the dictation/Siri troubleshooting for the on-device recognition path — see the recognition-ladder note above for why that matters.
