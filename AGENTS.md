...
**Build numbers come from App Store Connect, not from the repo.** `Tools/next_build_number.py` returns the highest number Apple has accepted, plus one, and `release.sh` stamps it at upload time. This exists because two counters were interleaving — `project.yml` for local uploads (1, 2, 3) and `CI_BUILD_NUMBER` for Xcode Cloud (18) — which is how a build "3" ended up landing after a build "18". App Store Connect requires the number to increase for a submission, so the two schemes could have blocked shipping. The value in `project.yml` is now only a local default; it is not what gets uploaded.

Historic note: bump `CFBundleVersion` in `project.yml` after every accepted upload, not before the next one — App Store Connect permanently rejects a build number it has already accepted, and bumping after means the repo always holds an unused number.
...
**The reading-line alignment self-heals in the ticker.** Discrete events are not enough: rotation produces transient layouts whose measurements can leave a stale offset, and afterwards nothing fires to correct it — the script ends up floating with the opening line scrolled off screen. The ticker re-checks every frame and corrects when off by more than 1pt. This is safe *only* because word frames are measured in the flow's own coordinate space and therefore do not move when `offset` does; measuring them in screen space would make this a feedback loop. Guarded by `!manualMode` and `!isDraggingLine` so it never fights a finger.
