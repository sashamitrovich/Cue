...
**The pursuit ceiling is measured against the *configured* pace, not the measured one.** `ScrollPursuit.speed` caps catch-up at `pursuitMaxCatchUp x` the speaking pace, and that pace comes from `state.targetWPM` — deliberately not `effectiveWPM`. `ReadingPace.effectiveWPM` is a whole-take average (`wordsRead / elapsed`, floored at 60) and `wordsRead` is the *cursor*, so any pause taken without hitting Pause drags it down and ad-libbing tanks it: the cursor stalls while the clock runs. Feeding that average to the ceiling made catch-up slower the longer a take ran, and slowest exactly when the matcher had the most ground to make up — reported as "laggy, was smoother in 1.3". The estimates and the wpm readout still use the measured average; that is what it is for. Note also that the ceiling binds past a gap of `pursuitResponse * pursuitMaxCatchUp * wpm/60` words — about 1.4 at the default pace, independent of font size — so it governs ordinary reading rather than guarding outliers. `ScrollPursuitTests` asserts both the burst-recovery time and the degradation, so a change to either constant will show up there.
